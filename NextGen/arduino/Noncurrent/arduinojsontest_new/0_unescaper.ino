...
//DELIMITER, ESCAPE, DELIMITER_HELMPER and ESCAP
E_HELPER must all be unique
...
const unsigned int DEFAULT_UNESCAPER_BUFFER_SIZE=1024;//The max unescaped message length. Bigger number takes up more memory but allows longer messages.

template<const int buffer_size=DEFAULT_UNESCAPER_BUFFER_SIZE>
class Unescaper
{
  private:
    bool escape=false;
...
void escape_and_send_message(void send(char),
                            char*message,
                            unsigned int length)
{
