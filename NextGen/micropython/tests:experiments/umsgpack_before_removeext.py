...
#   Removed support for "ext" handlers, because I'm communicating with c code and 
...
def _pack_ext(obj, fp, options):
    ...
    if   obj_len ==    1:fp.write(b"\xd4" + struct.pack("B", obj.type & 0xff) + obj.data)
    elif obj_len ==    2:fp.write(b"\xd5" + struct.pack("B", obj.type & 0xff) + obj.data)
    elif obj_len ==    4:fp.write(b"\xd6" + struct.pack("B", obj.type & 0xff) + obj.data)
    elif obj_len ==    8:fp.write(b"\xd7" + struct.pack("B", obj.type & 0xff) + obj.data)
    elif obj_len ==   16:fp.write(b"\xd8" + struct.pack("B", obj.type & 0xff) + obj.data)
    elif obj_len < 2** 8:fp.write(b"\xc7" +struct.pack( "BB", obj_len, obj.type & 0xff) + obj.data)
    elif obj_len < 2**16:fp.write(b"\xc8" +struct.pack(">HB", obj_len, obj.type & 0xff) + obj.data)
    elif obj_len < 2**32:fp.write(b"\xc9" +struct.pack(">IB", obj_len, obj.type & 0xff) + obj.data)
    else:
...
def pack(obj, fp, **options):
    ...
    elif ext_handlers and obj.__class__ in ext_handlers :_pack_ext(ext_handlers[obj.__class__](obj), fp, options)
    ...
    elif isinstance(obj, Ext)                           :_pack_ext(obj, fp, options)
    elif ext_handlers:
...
def _unpack_integer(code, fp, options):
    if   (ord(code) & 0xe0) == 0xe0:return struct.unpack( "b", code               )[0]
    elif code == b'\xd0'           :return struct.unpack( "b", _read_except(fp, 1))[0]
...
def _unpack_ext(code, fp, options):
    if   code == b'\xd4':length = 1
    elif code == b'\xd5':length = 2
    elif code == b'\xd6':length = 4
    elif code == b'\xd7':length = 8
    elif code == b'\xd8':length = 16
    elif code == b'\xc7':length = struct.unpack("B", _read_except(fp, 1))[0]
    elif code == b'\xc8':length = struct.unpack(">H", _read_except(fp, 2))[0]
    elif code == b'\xc9':length = struct.unpack(">I", _read_except(fp, 4))[0]
    else:
...
def _unpack_array(code, fp, options):
    if (ord(code) & 0xf0) == 0x90:length = (ord(code) & ~0xf0)
    elif code == b'\xdc'         :length = struct.unpack(">H", _read_except(fp, 2))[0],
...
for code in range(0xc7, 0xc9 + 1):_unpack_dispatch_table[struct.pack("B", code)] = _unpack_ext    # Ext
...
for code in range(0xd4, 0xd8 + 1):_unpack_dispatch_table[struct.pack("B", code)] = _unpack_ext    # Fixext
